    ...
    onError: (err) => {
      if(err instanceof AxiosError) {
        if(err.response?.status === 409) {
          return toast({
            title: 'LocateMe already exist.',
            description: 'Please choose a different LocateMe name.',
            variant: 'destructive',
          })
        }
      ...
      toast({
        ...
        description: 'Could not create LocateMe.',
        variant: 'destructive'
      })
    },
    ...
    <div className='relative bg-white w-full h-fit p-4 rounded-lg space-y-6'>
        ...
        <div>
            ...
            <div className='relative'>
                <p className='absolute text-sm left-0 w-8 inset-y-0 grid place-items-center text-zinc-400'>L/</p>
        ...
        <div className='flex justify-end gap-4'>
            ...
            onClick={() => createCommunity()}>Create Coummunity</Button>
        </div>
    </div>
  </div>
}
